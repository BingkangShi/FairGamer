...
def parse_args():
    ...
    parser.add_argument(
        "-m_t", "--model_type",
        type=str,
        help="Type of evaluated model. 'mllm' or 'llm'."
    )
    parser.add_argument(
        "-tm", "--test_mode",
        type=str,
        help="Test Mode, you can chose 'real' or 'virtual'."
    )
    parser.add_argument(
        "-gn", "--game_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=list,
        help="Evaluated languages." # use ["english"], "chinese" and "arabic" is not supported yet
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-temp", "--temperature",
        type=float,
        help="temperature for LLM" # use temperature=1.0 as default for all tasks
    )
    parser.add_argument(
        "-top_p", "--top_p",
        type=float,
        help="top_p for LLM" # use top_p=0.7 as default for all tasks
    )
    parser.add_argument(
        "-r", "--repeated_num",
        type=int,
