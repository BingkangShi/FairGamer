...
def calculate_averages_for_deci_vector(records, dict_keys):
    ...
    for sublist in records:
        ...
        values_dict = {key: [] for key in dict_keys}
        ...
        for key in dict_keys:
            if count_dict[key] > 0:
                ...
                if len(values_dict[key]) > 1:
                    std_dict[key] = np.std(values_dict[key], ddof=1)
                else:
                    std_dict[key] = 0.0
            else:
                average_dict[key] = 0
        ...
        if len(average_dict) == len(dict_keys):
            ...
            all_stds.append(std_dict)
            sampled_records.append(values_dict)
        else:
    ...
    sample_std = {}
    for key in dict_keys:
        stds_for_key = [s[key] for s in all_stds]
        if stds_for_key:
            sample_std[key] = np.mean(stds_for_key)
        else:
            sample_std[key] = 0.0

    return new_records, sample_std, sampled_records
...
async def Trade_evaluate(
    ...
    llm_config = config_list_from_json(env_or_file="../APIconfigure/configure.json", filter_dict=filter_dict)[0]
    print("llm_config: \n{}".format(llm_config))

    # Observation Space:
    if evaluated_language == "english":
        gaming_prompt = read_txt("./prompt/en_Tr_prompt.txt")
        # gaming_prompt = read_txt("./prompt/en_Tr_prompt_w_COT.txt")
        discount_class = read_txt("./prompt/en_discount_class.txt")
    elif evaluated_language == "chinese":
        gaming_prompt = read_txt("./prompt/ch_Tr_prompt.txt")
        # gaming_prompt = read_txt("./prompt/ch_Tr_prompt_w_COT.txt")
    ...
    new_records, sample_std, sampled_records = calculate_averages_for_deci_vector(reshaped_records, dict_keys=["discount"])
    print("sample_std: {}".format(sample_std))
    ...
    return versus_2d_records, all_responses, sample_std, sampled_records
...
async def multi_evaluate(
    ...
    record_with_lang = {}
    responses_with_lang = {}
    sample_std_with_lang = {}
    sampled_records_with_lang = {}
    for test_language in tqdm(evaluate_languages, desc="Loop", leave=False): # with different language
        print("test_language: {}\n".format(test_language))
        temp_record, temp_responses, temp_sample_std, temp_sampled_records = await Trade_evaluate(
        ...
        sample_std_with_lang[test_language] = temp_sample_std
        sampled_records_with_lang[test_language] = temp_sampled_records

    if "/" in model_name: 
        model_name_temp = model_name.split("/")[-1]
    else:
        model_name_temp = model_name
    json_dump_path = './record/Tr_{}_{}_{}_{}-avg_deci_vector.json'.format(bias_type, test_mode, game_name, model_name_temp)
    write_to_json(record_with_lang, json_dump_path)
    json_dump_path = './record/Tr_{}_{}_{}_{}-raw_responses.json'.format(bias_type, test_mode, game_name, model_name_temp)
    write_to_json(responses_with_lang, json_dump_path)
    json_dump_path = './record/Tr_{}_{}_{}_{}-sample_std.json'.format(bias_type, test_mode, game_name, model_name_temp)
    write_to_json(sample_std_with_lang, json_dump_path)
    json_dump_path = './record/Tr_{}_{}_{}_{}-sampled_records.json'.format(bias_type, test_mode, game_name, model_name_temp)
    write_to_json(sampled_records_with_lang, json_dump_path)

    print(record_with_lang)

    return record_with_lang, sample_std_with_lang, sampled_records_with_lang
