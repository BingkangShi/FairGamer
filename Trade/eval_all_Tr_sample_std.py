...
if __name__ == '__main__':
    ...
    for i in range(len(args_list)):
        ...
        record_with_lang, sample_std_with_lang, sampled_records_with_lang = asyncio.run(
            multi_evaluate(
        ...
        en_versus_matrix_2d = convert_to_2d_matrix(record_with_lang["english"], key_name="discount")
        ...
        ch_versus_matrix_2d = convert_to_2d_matrix(record_with_lang["chinese"], key_name="discount")
        ...
        # Calculate and print STDs
        print("\n--- STD Analysis ---")
        
        # English
        try:
            # Within-group mean STD (already calculated in Trade_sample_std.py)
            en_within_std = sample_std_with_lang["english"]["discount"]
            
            # Between-group STD (std of the averages)
            # record_with_lang["english"] is a 2D list of dicts. Flatten it to get all values.
            en_averages = [d["discount"] for sublist in record_with_lang["english"] for d in sublist if isinstance(d, dict) and "discount" in d]
            en_between_std = np.std(en_averages, ddof=1) if len(en_averages) > 1 else 0.0
            
            print(f"English - Within-group Mean STD: {en_within_std:.4f}")
            print(f"English - Between-group STD: {en_between_std:.4f}")
        except Exception as e:
            print(f"Error calculating English STDs: {e}")

        # Chinese
        try:
            # Within-group mean STD
            ch_within_std = sample_std_with_lang["chinese"]["discount"]
            
            # Between-group STD
            ch_averages = [d["discount"] for sublist in record_with_lang["chinese"] for d in sublist if isinstance(d, dict) and "discount" in d]
            ch_between_std = np.std(ch_averages, ddof=1) if len(ch_averages) > 1 else 0.0
            
            print(f"Chinese - Within-group Mean STD: {ch_within_std:.4f}")
            print(f"Chinese - Between-group STD: {ch_between_std:.4f}")
        except Exception as e:
            print(f"Error calculating Chinese STDs: {e}")
    ...
    json_dump_path = './record/Tr_{}_analysis.json'.format(model_name)
